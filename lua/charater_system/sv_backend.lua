...
local function sendCharacterModelToClient(ply)
    ...
    if json_data then
        ...
        if character_data and character_data.model then
            net.Start('SendCharacterModel')  -- Изменено имя сетевого сообщения
...
local function sendSecondCharacterModelToClient(ply)
    ...
    if json_data then
        local character_data = util.JSONToTable(json_data)
        if character_data and character_data.model then
            net.Start('Send2CharacterModel')  -- Изменено имя сетевого сообщения
            net.WriteString(character_data.model)
            net.Send(ply)
        end
    end
...
net.Receive('RequestCharacterModel', function(len, ply)
    ...
    sendCharacterInfo(ply)  -- Добавьте эту строку
end)

net.Receive('Request2CharacterModel', function(len, ply)
    sendSecondCharacterModelToClient(ply)
    sendSecondCharacterInfo(ply)  -- Добавьте эту строку
end)
...
local function updatecharacterinfo(ply)
    ...
    local charrank = ply:GetNWString('PlayerRank') or 'Курсант'
    local charspec = ply:GetNWString('Playerspec') or 'Стрелок'
    local charlevelpoints = ply:GetNWInt('PlayerLevelPoints') or 0
    local charlevel = ply:GetNWInt('PlayerLevel') or 0
    local charupgradepoints = ply:GetNWInt('PlayerUpgradePoints') or 0
    local charcur_armor_level = ply:GetNWInt('PlyCurArmorLevel') or 0
    local charcur_hp_level = ply:GetNWInt('PlyCurHPLevel') or 0
    ...
    local saved_data = {
        ...
        spec = charspec,
        level = charlevel,
        levelpoints = charlevelpoints,
        upgrade = charupgradepoints,
        hp_level = charcur_hp_level,
        arm_level = charcur_armor_level
    }
...
local function loadcharacterinfo(ply)
    ...
    if saved_data then
        ...
        if jobID and team.Valid(jobID) then
            ...
            print('Ошибка: Профессия с ID ' .. tostring(jobID) .. ' не найдена.')
        end
        ply:SetModel(saved_data.model or 'models/maintail.mdl')
        ...
        ply:SetNWInt('PlyCurArmorLevel', saved_data.arm_level or 0)
        ply:SetNWInt('PlyCurHPLevel', saved_data.hp_level or 0)
        ply:SetNWString('PlayerRank', saved_data.rank or 'Курсант')
        ply:SetNWString('Playerspec', saved_data.spec or 'Стрелок')
        ply:SetNWInt('PlayerLevelPoints', saved_data.levelpoints or 0)
        ply:SetNWInt('PlayerLevel', saved_data.level or 0)
        ply:SetNWInt('PlayerUpgradePoints', saved_data.upgrade or 0)
    else
...
local function createcharacter(ply,name)
    ...
    local charjob = '1tail'
    local charskin = 0
    local charmodel = 'models/maintail.mdl'
    local charrank = 'Курсант'
    local charspec = 'Стрелок'
    local charlevelpoints = ply:SetNWInt('PlayerLevelPoints', 0)
    local charlevel = ply:SetNWInt('PlayerLevel', 0)
    local charupgradepoints = ply:SetNWInt('PlayerUpgradePoints', 0)
    -- Ensure the main directory exists
    ensureDirectoryExists('character_system_data')
    ...
    if saved_data then
        ...
        ply:SetNWInt('PlyCurArmorLevel', saved_data.charcur_hp_level or 0)
        ply:SetNWInt('PlyCurHPLevel', saved_data.charcur_armor_level or 0)
        ply:SetNWString('PlayerRank', saved_data.rank or 'Курсант')
        ply:SetNWString('Playerspec', saved_data.spec or 'Стрелок')
        ply:SetNWInt('PlayerLevelPoints', saved_data.levelpoints or 0)
        ply:SetNWInt('PlayerLevel', saved_data.level or 0)
