...
local function plusik(ply)
    ...
    function dopPanelblyat()
        ...
        dopPanel.Paint = function (self, w, h)
            ...
            local textY = h * 0.25
            local texts = {
                'Придумайте себе имя, фамилию и позывной',
                'персонажу (важно, что бы придуманное ваше имя,',
                'фамилия и позывной были на кирилице, не,',
                'содержала латинских букв, цифр, др знаков)',
                '',
                'Пример: Чернышев Иван  "Шерлок"',
                'Пример: Моргунов Артём "Камень"',
                'Пример: Смернов Сергей "Ангел"'
            }
        ...
        textentrysuka:SetPlaceholderText('Введите текст...')
...
local function secondplusik(ply)
    ...
    function sdopPanelblyat()
        ...
        sdopPanel.Paint = function (self, w, h)
            draw.RoundedBox(0, 0, 0, w, h, Color(23, 23, 23, 225))
            draw.SimpleText('Фамилия, имя, позывной', 'font2', w * 0.7, h * 0.15, Color( 255, 255, 255, 255 ), 1,1)
...
local function characterexists(ply)
    ...
    nameLabel:SetText('Имя: ' .. (characterInfo.name or 'Загрузка...'))
    ...
    specLabel:SetText('Спец.: ' .. (characterInfo.spec or 'Загрузка...'))
    ...
    rankLabel:SetText('Ранг: ' .. (characterInfo.rank or 'Загрузка...'))
...
local function secondcharacterexists(ply)
    ...
    nameLabel:SetSize(buttonexistscharacter:GetWide() - 20, ScreenScale(5))
    nameLabel:SetFont('font1')
    nameLabel:SetText('Имя: ' .. (secondcharacterInfo.name or 'Загрузка...'))
    ...
    specLabel:SetSize(buttonexistscharacter:GetWide() - 20, ScreenScale(5))
    specLabel:SetFont('font1')
    specLabel:SetText('Спец.: ' .. (secondcharacterInfo.spec or 'Загрузка...'))

    local rankLabel = vgui.Create('DLabel', buttonexistscharacter)
    rankLabel:SetPos(10, buttonexistscharacter:GetTall() * 0.82)
    rankLabel:SetSize(buttonexistscharacter:GetWide() - 20, ScreenScale(5))
    rankLabel:SetFont('font1')
    rankLabel:SetText('Ранг: ' .. (secondcharacterInfo.rank or 'Загрузка...'))
